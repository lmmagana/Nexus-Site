...
async function setCreditBalance(newValue, contactId, extendedField, retryCount = 0) {
    try {
        ...
        if (error?.description?.includes("Contact has been updated since the requested revision") ||
            ...
            if (retryCount < 3) {
                console.warn(`🔁 Retrying update (attempt ${retryCount + 1})...`);
            ...
            console.error("Couldn't add sessions", error);
        }
    }
}
...
export const invoke = async ({payload}) => {
  ...
  try {
    if(planName.includes("Private")) {
      setCreditBalance("0", contactId, privatesExtendedField);
      console.log(`Plan: ${planName} cancelled/ended and updated with 0 privates for ${memName}`);
    }
    else if(planName.includes("Intro")) {
      setCreditBalance("0", contactId, privatesExtendedField);
      setCreditBalance("0", contactId, sessionsExtendedField);
      console.log(`Plan: ${planName} cancelled/ended and updated with 0 sessions & privates for ${memName}`);
    }
    else if (planName.includes("Punch")) {
      setCreditBalance("0", contactId, sessionsExtendedField);
      console.log(`Plan: ${planName} cancelled/ended and updated with 0 sessions for ${memName}`);
    }
  } catch (error){
    console.log(`Could not ensure sessions were 0 for ${memName} after cancelling/ending ${planName}`, error)
  }
